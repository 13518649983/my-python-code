...
T = 1
epochs = 100  # 迭代次数
num_features = 9  # 输入特证数
embed_dim = 32  # 嵌入维度
dense_dim = 32  # 隐藏层神经元个数
num_heads = 4  # 头数
dropout_rate = 0.001  # 失活率
num_blocks = 3  # 编码器解码器数
learn_rate = 0.0005  # 学习率
batch_size = 32  # 批大小

# 读取数据
dataset = pd.read_excel('dy1-test.xlsx', usecols=[0, 1, 2, 3, 4,5,6,7,8]) #文件名以及排数
dataX = dataset.values
dataY = dataset['PP'].values
...
def phy_loss(predictions,inputs):
    phy_loss = nn.MSELoss(predictions,inputs)
    return phy_loss
...
df.to_excel('D:\pythonProjects\Paper\LSTM-Transformer\moxing\predictions.xlsx',index=False)
# 计算模型的评价指标
